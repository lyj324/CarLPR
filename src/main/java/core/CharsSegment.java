...
public class CharsSegment {
    public int charsSegment(final opencv_core.Mat input, Vector<opencv_core.Mat> resultVec) {
        ...
        opencv_core.Mat input_grey = new opencv_core.Mat();
        cvtColor(input, input_grey, CV_RGB2GRAY);

        int w = input.cols();
        int h = input.rows();
        opencv_core.Mat tmpMat = new opencv_core.Mat(input, new opencv_core.Rect((int) (w * 0.1), (int) (h * 0.1), (int) (w * 0.8), (int) (h * 0.8)));
        ...
        // 找轮廓
        opencv_core.Mat img_contours = new opencv_core.Mat();
        ...
        // 将不符合特定尺寸的图块排除出去
        Vector<opencv_core.Rect> vecRect = new Vector<opencv_core.Rect>();
        for (int i = 0; i < contours.size(); ++i) {
            opencv_core.Rect mr = boundingRect(contours.get(i));
            if (verifySizes(new opencv_core.Mat(img_threshold, mr))) {
                vecRect.add(mr);
            }
        }
        ...
        Vector<opencv_core.Rect> sortedRect = new Vector<opencv_core.Rect>();
        // 对符合尺寸的图块按照从左到右进行排序
        ...
        if (this.isDebug) {
            if (specIndex < sortedRect.size()) {
                opencv_core.Mat specMat = new opencv_core.Mat(img_threshold, sortedRect.get(specIndex));
        ...
        // 退过特定算法来指定
        opencv_core.Rect chineseRect = new opencv_core.Rect();
        if (specIndex < sortedRect.size()) {
        ...
        if (this.isDebug) {
            opencv_core.Mat chineseMat = new opencv_core.Mat(img_threshold, chineseRect);
        ...
        Vector<opencv_core.Rect> newSortedRect = new Vector<opencv_core.Rect>();
        ...
        for (int i = 0; i < newSortedRect.size(); i++) {
            opencv_core.Rect mr = newSortedRect.get(i);
            opencv_core.Mat auxRoi = new opencv_core.Mat(img_threshold, mr);
     ...
     * @return
     */
    private Boolean verifySizes(opencv_core.Mat r) {
     ...
     * @return
     */
    private opencv_core.Mat preprocessChar(opencv_core.Mat in) {
        int h = in.rows();
        int w = in.cols();
        int charSize = CHAR_SIZE;
        opencv_core.Mat transformMat = opencv_core.Mat.eye(2, 3, CV_32F).asMat();
        int m = Math.max(w, h);
        transformMat.ptr(0, 2).put(Convert.getBytes(((m - w) / 2f)));
        transformMat.ptr(1, 2).put(Convert.getBytes((m - h) / 2f));

        opencv_core.Mat warpImage = new opencv_core.Mat(m, m, in.type());
        warpAffine(in, warpImage, transformMat, warpImage.size(), INTER_LINEAR, BORDER_CONSTANT, new opencv_core.Scalar(0));

        opencv_core.Mat out = new opencv_core.Mat();
        resize(warpImage, out, new opencv_core.Size(charSize, charSize));
     ...
     * @return
     */
    private opencv_core.Mat clearLiuDing(opencv_core.Mat img) {
        final int x = this.liuDingSize;

        opencv_core.Mat jump = opencv_core.Mat.zeros(1, img.rows(), CV_32F).asMat();
     ...
     * @return
     */
    private opencv_core.Rect GetChineseRect(final opencv_core.Rect rectSpe) {
        ...
        opencv_core.Rect a = new opencv_core.Rect(newx, y, (int) newwidth, height);
        return a;
    }
     ...
     * @return
     */
    private int GetSpecificRect(final Vector<opencv_core.Rect> vecRect) {
        ...
        int specIndex = 0;
        for (int i = 0; i < vecRect.size(); i++) {
            opencv_core.Rect mr = vecRect.get(i);
     ...
     * @return
     */
    private int RebuildRect(final Vector<opencv_core.Rect> vecRect, Vector<opencv_core.Rect> outRect, int specIndex) {
     ...
     * @return
     */
    private void SortRect(final Vector<opencv_core.Rect> vecRect, Vector<opencv_core.Rect> out) {
